...
namespace Assets.C_Script
{
    public class CS_MeshCutter
    {
        ...
        private static CS_MeshCutter left_side = new CS_MeshCutter();
        private static CS_MeshCutter right_side = new CS_MeshCutter();
        ...
        public static GameObject[] Cut(string name,GameObject victim, Vector3 anchorPoint, Vector3 normalDirection, Material capMaterial)
        {
            ...
            {
                if (!leftSideObj.TryGetComponent<BoxCollider>(out var box)) 
                {
                    box = leftSideObj.AddComponent<BoxCollider>(); 
                }
                if (!leftSideObj.TryGetComponent<Rigidbody>(out var rigidbody))leftSideObj.AddComponent<Rigidbody>();
                Bounds bounds = left_HalfMesh.bounds;
                box.center = bounds.center;
                box.size = bounds.size;
            }
            GameObject rightSideObj = new GameObject(name+"right side", typeof(MeshFilter), typeof(MeshRenderer),typeof(BoxCollider),typeof(Rigidbody));
            {
                rightSideObj.transform.position = victim.transform.position;
                rightSideObj.transform.rotation = victim.transform.rotation;
                rightSideObj.GetComponent<MeshFilter>().mesh = right_HalfMesh;
                Bounds bounds = right_HalfMesh.bounds;
                BoxCollider box = rightSideObj.GetComponent<BoxCollider>();
                box.center = bounds.center;
                box.size = bounds.size;
            }
            // assign mats
